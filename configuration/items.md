...
| `transform:<SERVICE>`                                                                         | State   | All                   | Transformation Profiles can be used to transform the item stat (at channel-link level instead of only transforming the visual representation) using the specified transformation service. You can find the documentation of these profiles within the [transformations docs](/docs/configuration/transformations.html) for script transformations or in the [transformation add-on docs](/addons/#transform), e.g. [Map](/addons/transformations/map/#usage-as-a-profile) or [JsonPath](/addons/transformations/jsonpath/#usage-as-a-profile) |
