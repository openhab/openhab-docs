...
| `non-undef`                                                                                   | State   | Color, Dimmer, Switch | This Profile converts non-`UNDEF` and non-`NULL` states and commands from the Channel to Item as `ON`. All other values are converted to `OFF`. Item Commands are passed as-is towards ThingHandler. One can have inverse logic (`ON` and `OFF` flipped) using the `inverted` (optional) `boolean` parameter.                                                                                                                                                                                 |
