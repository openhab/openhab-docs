...
> These are not runtime rules; they do not react to live state changes in openHAB.
...
Evaluates multiple conditions in order and stops at the first "truthy" match.
...
When a value is used in a conditional, it is first evaluated and then interpreted as either _truthy_ or _falsy_.
...
The preprocessor only processes the active branch.
Tags (like `!include`) inside inactive branches are ignored.
