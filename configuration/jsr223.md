...
The `cache` preset does not provide a default import and needs to be imported explicitely.
...
In both cases values that are either a `ScheduledFuture<?>` or a `Timer` are cancelled by calling `.cancel()` on the object.

Both caches implement the `ValueCache` interface and therefore can be accessed by

- `put(String key, Object value)`: Put a key/value pair to the cache.
- `remove(String key)`: Remove the key/value pair from the cache.
- `get(String key)`: Get the value for the given key from the cache. Non-existent keys return `null`.
- `get(String key, Supplier<Object> supplier`: Get the value for the given key. If no value is present, add the value that is return from the `supplier`.
