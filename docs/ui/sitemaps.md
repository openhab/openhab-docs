---
title: Sitemaps
source: https://github.com/openhab/openhab-docs/blob/main/src/ui/sitemaps.md
meta:
  - property: og:title
    content: "Sitemaps"
...
If you are a new user, it's probably a good idea to start [customizing your Overview page first](/docs/tutorial/auto_overview.html#customization-page-configuration).
:::

In openHAB a collection of [Things](/docs/concepts/things.html) and [Items](/docs/concepts/items.html) represent physical or logical objects in the user's home automation setup.
Sitemaps are one way to select and compose these elements into a user-oriented representation for various User Interfaces (UIs), including the [openHAB app for Android](/docs/apps/android.html).
...
For easy editing of sitemap definition files, we suggest to use one of the [openHAB supporting editors](/docs/configuration/editors.html).
...
Data presented by Sitemap elements will almost always originate from a referenced [Item](/docs/configuration/items.html).
...
- Common parameters, also known from [items definition](/docs/configuration/items.html#item-syntax):
  - `item` defines the name of the Item you want to present (e.g. `Temperature`), [more details](/docs/configuration/items.html#item-name).
  - `label` sets the textual description displayed next to the preprocessed Item data (e.g. "`Now [%s °C]`"), [more details](/docs/configuration/items.html#item-label).
  - `icon` chooses the icon to show next to the element, [more details](/docs/configuration/items.html#icons).

- When an [Item](/docs/configuration/items.html) is defined, you have the opportunity to assign a label and/or an icon at that point.
...
UoM = [Units of Measurement](/docs/concepts/units-of-measurement.html)
...
Please refer to the documentation on [item State Presentation](/docs/configuration/items.html#state-presentation) for details.
...
A Group element can be defined with or without child widgets.

If no child elements are defined, clicking on a Group element will reveal a new view showing all group items using the [Default](#element-type-default) element type.
In this case, there is no way to override the parameters, change the default element type, change the order, use dynamic tags, or insert other elements (e.g. Chart, Image, Webview, etc) in the subframe generated by the Group element.

If child widgets are defined, all Group Item children will not be shown anymore unless defined as a child widget.

In addition, Item groups may be configured to hold a value, just as with normal items.
Please refer to the documentation on [Item groups](/docs/configuration/items.html#groups) for details.
...
All Sitemap elements can be configured to be hidden, color highlighted or to have a [dynamic icon](/docs/configuration/items.html#dynamic-icons), depending on certain Item states.
...
- Present a special icon, depending on the state of an item (a [dynamic icon](/docs/configuration/items.html#dynamic-icons))
...
Please refer to the documentation on [Item configuration](/docs/configuration/items.html#icons) for details.
