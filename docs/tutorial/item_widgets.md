...
source: https://github.com/openhab/openhab-docs/blob/main/src/tutorial/item_widgets.md
meta:
  - property: og:title
    content: "Pages - Item Widgets"
...
For full details on expressions see the [Expressions docs](/docs/ui/widget-expressions-variables.html).
...
See the [UI docs](/docs/ui/building-pages.html) for details on how to create custom widgets.
...
See the [widget reference docs](/docs/ui/components/index.html) for details.
...
Pattern | Defines the pattern used to display the state. This is where you will define the transformation and any other formatting information using the same [syntax used by sitemaps](/docs/configuration/items.html#state-presentation). Everything that can go between the `[ ]` in an Item label as described by that doc (excluding the `[ ]` themselves) can go here.
...
See the [UI Docs for Actions](/docs/ui/building-pages.html#actions) for details on all of the Actions that can be performed.
