...
source: https://github.com/openhab/openhab-docs/blob/main/src/tutorial/model.md
meta:
  - property: og:title
...
Once created, a model allows one to use [Semantics Actions](/docs/configuration/actions.html#semantics) in Rules to e.g. determine the Location of an Item or the related Equipment.
...
Clicking on this offers two options: "Add Items from textual definition" which allows one to import one or more Items based on the [textual configuration syntax](/docs/configuration/items.html), and "Add Item" which allows the creation of the UI by filling in a form.
