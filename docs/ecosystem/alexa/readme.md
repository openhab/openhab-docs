...
meta:
  - property: og:title
    content: "openHAB Skill For Amazon Alexa"
  - property: og:description
    content: "Alexa is an intelligent personal assistant developed by Amazon and designed to run on smart speakers and devices such as the Amazon Echo and Echo Dot."
---
...
Single items in openHAB can be mapped to a single endpoint in Alexa through the use of the [item metadata](/docs/concepts/items.html#item-metadata).
...
Only [functional groups](/docs/configuration/items.html#groups) (no group type) can be configured as group endpoint. [State derived groups](/docs/configuration/items.html#derive-group-state-from-member-items) will be considered as single endpoints based on their defined group type.
...
Item states, reported back to Alexa, for numerical item types are formatted based on their [item state presentation](/docs/configuration/items.html#state-presentation) definition if configured. This means you can control the precision of number values (e.g. `%.1f °C` will limit reported temperature value to one decimal point).

For items that don't have a state, these can be configured as not retrievable, automatically when the item [parameter `autoupdate`](/docs/configuration/items.html#parameter-autoupdate) is set as `autoupdate="false"` or by using metadata parameter `retrievable="false"`. In that case, the skill will not report back the state of the given item to Alexa. It is important to note that this will affect the usability of some of the advanced features in the Alexa app that require state reporting.
...
With the introduction of the [unit of measurement](/docs/concepts/units-of-measurement.html) concept, the item unit can be automatically determined for thermostat and temperature using that feature, removing the need of having to set the metadata scale parameter for each of the relevant items or groups.
...
* Supported metadata parameters:
  * binding=`<id>`
    ...
    * only for [GUI-based configuration](/docs/configuration/)
