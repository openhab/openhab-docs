...
The available categories correspond with the [available icons of the classic iconset](/docs/configuration/iconsets/classic/), however categories are written in Java class-naming style, e.g. `FrontDoor` instead of lowercase `frontdoor`.
...
The available categories correspond with the [available icons of the classic iconset](/docs/configuration/iconsets/classic/), however categories are written in Java class-naming style, e.g. `BatteryLevel` instead of lowercase `batterylevel`.
...
The former are used by the openHAB User Interface to create automatic groupings of items according to the [Semantic Model](/docs/tutorial/model.html).
...
1. For further reading please see the [Description of the Semantic Model](/docs/tutorial/model.html), the [Developer Guidelines on Semantic Tags](semantic-tags.md) and the [Thing-Type and Channel-Type validation schema](https://www.openhab.org/schemas/thing-description-1.0.0.xsd).
