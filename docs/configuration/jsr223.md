...
source: https://github.com/openhab/openhab-docs/blob/main/src/configuration/jsr223.md
meta:
  - property: og:title
    content: "JSR223 Scripting"
...
also known as [JavaScript Scripting](/addons/automation/jsscripting/), which includes the latest **ES6** language features, supports third party node libraries (please keep in mind that not all NodeJS APIs are available in GraalJS),
...
This provides great flexibility for the users of JSR223, but is also _use at your own risk_, since changes outside the official APIs occur frequently, and are not considered to be _breaking changes_.
...
This directory will vary, [based on the type of openHAB installation used](/docs/installation/linux.html#installation).
...
Script files from the above location are loaded on openHAB startup.
...
Upon reload of a script file, the old script is unloaded first and then the new script is loaded.

Other places in openHAB where scripting is used, e.g. transformations, might have a different script lifecycle.

## `ScriptExtension` Presets
...
Imported by default (`importPreset` not required).
...
| `UnDefType`             | [`org.openhab.core.types.UnDefType`](https://www.openhab.org/javadoc/latest/org/openhab/core/types/undeftype)                                                                                       |
| `NULL`                  | `UnDefType` enum item                                                                                                                                                                                       |
| `UNDEF`                 | `UnDefType` enum item                                                                                                                                                                                       |
| `RefreshType`           | [`org.openhab.core.types.RefreshType`](https://www.openhab.org/javadoc/latest/org/openhab/core/types/refreshtype)                                                                                   |
...
Refer to [Developer: Event Bus](/docs/developer/events.html#event-source) for more information about event source.
...
Provides a mechanism to execute a callback on disposal.
A script is disposed both on modification and removal, see [Script Lifecycle](#script-lifecycle).
...
This is demonstrated in the [`scriptLoaded` and `scriptUnloaded` functions](#scriptloaded-and-scriptunloaded-functions) section.
...
With the `add(...)` method, elements added over the above instances are removed when the script that inserted the elements is unloaded.
...
## `scriptLoaded` and `scriptUnloaded` functions
