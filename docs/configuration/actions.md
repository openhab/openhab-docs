---
title: Actions
source: https://github.com/openhab/openhab-docs/blob/main/src/configuration/actions.md
meta:
  - property: og:title
    content: "Actions"
...
They can be found in the article about [openHAB multimedia](/docs/configuration/multimedia.html).
...
One can configure whether specific log entries are logged out and where they get logged to (e.g. to a separate file) by [editing the logger configuration](/docs/administration/logging.html).
...
It contains [Thing Status](/docs/concepts/things.html), [Status Details](/docs/concepts/things.html) and [Status Description](/docs/concepts/things.html).
Refer to [Thing Status API](/docs/concepts/things.html) for how to get those information.
If you just want to know the status, you can use `thingStatusInfo.getStatus().toString()` and the result will be one of the values in [Thing Status](/docs/concepts/things.html).
...
openHAB has several subsystems that can be accessed from Rules. These include persistence, see [Persistence Extensions in Scripts and Rules](/docs/configuration/persistence.html#persistence-extensions-in-scripts-and-rules), transformations, scripts.
...
See [Transformations](/docs/configuration/transformations.html#usage) for available transformations and their usage.
