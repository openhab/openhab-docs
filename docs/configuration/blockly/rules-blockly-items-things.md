...
source: https://github.com/openhab/openhab-docs/blob/main/src/configuration/blockly/rules-blockly-items-things.md
meta:
  - property: og:title
    content: "Rules Blockly - Items & Things"
...
_Items_ and _Things_ are the [major entities of openHAB](/docs/concepts/) to control and monitor the home.
...
- Learn more about items [here](/docs/configuration/items.html)
...
- Returns the [state](/docs/configuration/items.html#state) of an item like ON/OFF, the temperature value etc.
...
See the [Item-State documentation](/docs/configuration/items.html#state) for more information
...
If you need an item that has one of multiple tags, then you need to use one block of each and then use the ["concatenate list block"](/docs/configuration/blockly/rules-blockly-standard-ext.html#concatenate-list) to combine the results.
...
For the difference between _send command_ and _post update_ see ["Manipulating States"](/docs/configuration/rules-dsl.html#manipulating-item-states) and ["Event Bus Actions"](/docs/configuration/actions.html#event-bus-actions).
...
- Learn more about [things](/docs/configuration/things.html) or [thing-concepts](/docs/concepts/things.html)
...
- Clicking 'MyThing' displays a list of **Things** to pick from.
- Technically this block returns a [ThingStatus](/docs/concepts/things.html#thing-status)  - a String with one of the following statuses
  - UNINITIALIZED
  - INITIALIZING
  - UNKNOWN
  - ONLINE
