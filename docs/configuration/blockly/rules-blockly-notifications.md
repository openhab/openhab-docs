...
source: https://github.com/openhab/openhab-docs/blob/main/src/configuration/blockly/rules-blockly-notifications.md
meta:
  - property: og:title
    content: "Rules Blockly - Notifications"
...
General information on cloud notification actions can be found at [Actions: Cloud Notification Actions](/docs/configuration/actions.html#cloud-notification-actions).
...
Use the [icon names of the openHAB iconset](/docs/configuration/iconsets/classic/).
...
To see all icons that can be used, see the table [openHAB icons](/docs/configuration/icons.html).
...
For convenience, it automatically connects a "send command" block to the notification block which requires to be configured (it can be replaced by [open / call Page Block](/docs/configuration/blockly/rules-blockly-notifications.html#actions-open-call-page) instead later on):
...
Please note that on iOS, a _tap & hold_ on the notification is needed to make the buttons visible.
