---
title: Items
source: https://github.com/openhab/openhab-docs/blob/main/src/configuration/items.md
meta:
  - property: og:title
    content: "Items"
...
1. Through text `.items` files located in the `$OPENHAB_CONF/items` folder.
    ...
    Refer to the [installation docs](/docs/installation/index.html) to determine your specific installation's folder structure.
...
It's recommended to edit `.items` files using one of the [openHAB supporting editors](/docs/configuration/editors.html).
...
[Item Types Overview](/docs/concepts/items.html)
...
Graphical UIs will display the label text when the Item is included, e.g. in Basic UI in a [Sitemap](/docs/ui/sitemaps.html) definition.
...
Channel labels can be overwritten by Item definitions and Item labels can be overwritten in [Sitemaps](/docs/ui/sitemaps.html#element-types).
...
_N.B._  Many openHAB users find that it can be very useful to use [Persistence](/addons/#persistence) and [System started](/docs/configuration/rules-dsl.html#system-based-triggers) rules so that their systems behaves in a predictable way after an openHAB restart.
...
You insert this Item into a [sitemap](/docs/configuration/sitemaps).
...
These relationships can be exploited in [Sitemaps](/docs/ui/sitemaps.html) or in [automation rules](/docs/configuration/rules-dsl.html) to navigate through the hierarchically organized Items or to perform computations and updates on subsets of similar Items.
...
Tags are used by the [Semantic Model](/docs/tutorial/model.html).
...
openHAB introduces the concept of [Things and Channels](/docs/concepts/things.html).
...
and a [Rule](/docs/configuration/rules-dsl.html) to toggle this light with a serial button:
