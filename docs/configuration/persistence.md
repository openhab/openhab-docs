---
title: Persistence
source: https://github.com/openhab/openhab-docs/blob/main/src/configuration/persistence.md
meta:
  - property: og:title
    content: "Persistence"
...
To select a default persistence service via UI, refer to the [Main UI docs](/docs/mainui/settings/persistence.html).
...
- from the UI, refer to the [Main UI docs](/docs/mainui/settings/persistence.html).
...
See the [Rules article](/docs/configuration/rules-dsl.html#time-based-triggers) for more information.
...
PowerMeter.persistedState(now.truncatedTo(ChronoUnit.DAYS).withDayOfMonth(1))
```
