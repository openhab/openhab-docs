---
title: Rules DSL
source: https://github.com/openhab/openhab-docs/blob/main/src/configuration/rules-dsl.md
meta:
  - property: og:title
    content: "Rules DSL"
...
Note that there is also a visual way of programming openHAB rules, which may be more suitable for beginners. Its documentation can be found in the [Blockly Reference section](/docs/configuration/blockly/)
...
The [openHAB VS Code Extension](/docs/configuration/editors.html#openhab-vs-code-extension) offers support for rules building.
...
Therefore, if the Rule needs to know what the command was, use the [implicit variable](/docs/configuration/rules-dsl.html#implicit-variables-inside-the-execution-block) `receivedCommand` instead of `<ItemName>.state`.
...
All of the [implicit variables](/docs/configuration/rules-dsl.html#implicit-variables-inside-the-execution-block) get populated using the Item that caused the event.
...
If the Rule needs to know what the triggering thing was, or access a string value of the previous or new status, use the [implicit variables](/docs/configuration/rules-dsl.html#implicit-variables-inside-the-execution-block) `triggeringThing`, `previousThingStatus` or `newThingStatus` to access the information.
...
If the Rule needs to know what the received event or the triggering channel was, use the [implicit variable](/docs/configuration/rules-dsl.html#implicit-variables-inside-the-execution-block) `receivedEvent` or `triggeringChannel` to access the information.
...
- all [standard actions](/docs/configuration/actions.html) to make something happen
- [persistence extensions](/docs/configuration/persistence.html#persistence-extensions-in-scripts-and-rules) to work with persisted item states
- [multimedia actions](/docs/configuration/multimedia.html) to work with audio, voice and text
- thing actions, see documentation for the specific binding
- access to a [cache](/docs/configuration/jsr223.html#dsl) that can preserve values accross rules and rule executions
...
In relation to [event-based rule triggers](/docs/configuration/rules-dsl.html#event-based-triggers) the manipulator commands `sendCommand` and `postUpdate` act differently.
...
A complete and up-to-date list of item types are currently allowed in openHAB and the command types each item can accept is given in the [openHab documentation for items](/docs/concepts/items.html).
...
The section in the [item documentation on formatting](/docs/concepts/items.html#state-and-command-type-formatting) details the requirements for the formatting.
...
_Reminder: For a complete and up-to-date list of what item types are currently allowed in openHAB and the command types each item can accept refer to the section on [items in the openHAB documentation](/docs/concepts/items.html)._
