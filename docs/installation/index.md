...
source: https://github.com/openhab/openhab-docs/blob/main/src/installation/index.md
meta:
  - property: og:title
...
To find out where these locations are, e.g. where `$OPENHAB_CONF` points to, use the [console](/docs/administration/console.html):

```shell
openhab-cli info
```

will result in e.g. for an [openHABian](/docs/installation/openhabian.html) installation
...
- Install [Visual Studio Code](https://code.visualstudio.com/Download) and the [openHAB VS Code Extension](/docs/configuration/editors.html#openhab-vs-code-extension) on your local machine, to manage your (remote) configuration files.
...
the [Beginner Tutorial](/docs/tutorial)
