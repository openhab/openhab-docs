---
title: Introduction
source: https://github.com/openhab/openhab-docs/blob/main/src/index.md
meta:
  - property: og:title
...
| Things | are the first openHAB (software) generated representation of your devices | [click for more info on Things](/docs/concepts/things.html) |
| Channels | are the openHAB (software) connection between “Things” and “Items” (see below) | _see below_ |
| Items | are the openHAB (software) generated representation of information about the devices | [click for more info on Items](/docs/concepts/items.html) |
| Rules | that perform automatic actions (in its simplest form: if "this" happens, openHAB will do "that") | [click for more info on Rules](/docs/configuration/rules-dsl.html) |
| Pages | is the openHAB (software) generated user interface (web site) that presents information and allows for interactions | [click for more info on Pages](/docs/tutorial/pages_intro.html) |
...
All the above [Concepts](/docs/concepts/index.html) are explained in more depth on other pages that can be accessed either through the sidebar or the links in the table above.

### Channels

Channels are the logical link between a [Thing](/docs/concepts/things.html) and an [Item](/docs/concepts/items.html).
Channels originate from [Things](/docs/concepts/things.html) definition and define how your [Thing](/docs/concepts/things.html) can communicate with [Item](/docs/concepts/items.html) (and vice versa).
You will create channels when defining your [Thing](/docs/concepts/things.html).

During the definition of your [Thing](/docs/concepts/things) you will identify the channel to which your [Item](/docs/concepts/items.html) will be linked.
These two steps ensure that openHAB can transmit the information from the [Thing](/docs/concepts/things.html) to the [Item](/docs/concepts/items.html) (and vice versa).

### Bindings

Bindings are software packages that are installed by the user in openHAB.
The main purpose of Bindings is to establish the connection between your device and your [Thing](/docs/concepts/things.html).
Bindings communicate with your device and translate all commands to and from openHAB between your device and your [Thing](/docs/concepts/things.html).
...
For each binding, detailed instructions and examples are provided that include guidance on configuration (if any) of the binding itself, the definition of [Things](/docs/concepts/things.html) supported by this binding and the Channels these [Things](/docs/concepts/things) provide.
In most cases, the description also contains a fully worked out example that includes a definition of [Things](/docs/concepts/things) and its Channels, [Items](/docs/concepts/items.html) linked to those Channels and the use of these [Items](/docs/concepts/items.html) in a user interface page.

## While You Are Getting Started

To all newcomers: please read the section [New User Tutorial](/docs/tutorial).
...
You can find specific installation instructions for these and other platforms in the [Installation Overview](/docs/installation/index.html) article.
...
If you have no strong preference, get a [Raspberry Pi 4](https://www.raspberrypi.org/products/raspberry-pi-4-model-b/) with a minimum of 2 and no more than 4 GB of RAM and an 'Endurance' labelled type of SD card and install [openHABian](/docs/installation/openhabian.html) for the best experience.
...
Once you have openHAB up and running, the [Configuration](/docs/configuration/index.html) article contains everything you need to know to get your openHAB installation talking to different devices around your home.

You will quickly discover that you may want to learn more about Things, Channels, Items, and more.
To do so, we highly recommend that you read the next chapter titled [Concepts](/docs/concepts/index.html).
...
If you are new to openHAB, we suggest you continue to the [Concepts](/docs/concepts/index.html) chapter where we introduce many fundamental ideas that are used throughout openHAB.
