...
Please be aware that this communication uses the public network so make sure your openHAB installation is [secured](/docs/installation/security.html#encrypted-communication) (but accessible from public internet through myopenhab.org or using a reverse proxy) and you configured HTTP**S** access in tracking applications.
...
In order to see detailed debug information [set TRACE debug level](/docs/administration/logging.html) for `org.openhab.binding.gpstracker` package.
