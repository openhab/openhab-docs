...
| LED Indicator Intensity (when off) | config_decimal | config_decimal |  | Number | 
...
The following table provides a summary of the 21 configuration parameters available in the LZW31.
...
| 13 | LED Indicator Color | Sets the default color of the LED bar |
...
| 52 | Smart Bulb Mode | Disable switching the load off in Smart Bulb Mode |
...
Calculated by using a hue color circle ( value / 360*255) see website for more detail
...
|--------|-------------|
| 0 | Single-Pole |
| 1 | Multi-Pole w/ Dumb Switch |
| 2 | Multi-Pole w/Smart Aux Switch |

The manufacturer defined default value is ```0``` (Single-Pole).
...
Disable switching the load off in Smart Bulb Mode
If it set to 1, the power will output maximum % when the dimmer is on. 0 = normal operation, 1 = smart bulb.  


Version 1.47 and newer
Values in the range 0 to 1 may be set.
