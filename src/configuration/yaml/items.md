...
| `type`       | Valid [item type](/configuration/items.html#type) (mandatory, case-insensitive).                                                                                                                                                                                                |
| `dimension`  | For dimensioned `Number` items, the `item type` must be set to just `Number`, and the dimension is set separately using this key. Valid dimensions are listed under the **Type** column in the [List of Units](/concepts/units-of-measurement.html#list-of-units) table.        |
| `group`      | This key allows defining the group type, dimension and function. For more details, see [Derive Group State from Member Items](/configuration/items.html#derive-group-state-from-member-items).                                                                                  |
...
| `icon`       | The item's [icon](/configuration/items.html#icons). The syntax is described in [icon-sources](/configuration/items.html#icon-sources).                                                                                                                                  |
...
> For example, [MAP Profile](/addons/transformations/map/#usage-as-a-profile) requires
> a `function` parameter.
>
> For more information on Profiles, see:
>
> - [Item-Channel Link Profile](/configuration/items.html#profiles)
> - [Script Transformation Profile](/configuration/transformations.html#script-transformation-profile)
