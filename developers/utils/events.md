...
Note that sources can be provided by the user (either as parameters to REST API calls, or in rule actions), and thus no assumptions should be made about what values might be seen, including things like "all events triggered by rules flow through `org.openhab.core.automation`" or "if a source indicates it came from a given bundle, that bundle actually triggered the event."
