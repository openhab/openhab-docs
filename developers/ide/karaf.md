...
This procedure allows for testing and debugging while running openHAB under [Apache Karaf](https://karaf.apache.org/).
...
Make sure that the bundles you want to test or debug have been installed in your local Maven repository, using `mvn clean install -pl :<bundle coordinates>` or similar. Any bundles not found in your local Maven repository will be downloaded in their latest snapshot version. If you want to test or debug an older version, you must therefore build and install everything into your local Maven repository first.  
...
The result of the build can be found in `openhab-distro/distributions/openhab/target/openhab-<version>`. From this folder, you can launch openHAB under Karaf by running `start` or `start_debug`.
