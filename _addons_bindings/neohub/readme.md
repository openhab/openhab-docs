...
Once the configuration parameters are all valid, the NeoHub Thing will automatically attempt to connect and sign on to the hub.
If the sign-on succeeds, the Thing status changes to ONLINE; otherwise it will show an error status.

Once the NeoHub Thing has been created and it has successfully signed on, it will automatically interrogate the NeoHub to discover all the respective Heatmiser device Things connected to it.
...
The NeoHub Thing connects to the hub (bridge) to communicate with connected Heatmiser device Things.
...
The NeoHub supports two APIs (an older legacy API and a modern one), and this binding can use either.
...
From early 2022 Heatmiser introduced NeoHub firmware that can enable or disable connecting to it via a TCP port.
...
Alternatively, the Heatmiser mobile app has a setting (Settings | System | API Access | Legacy API Enable | On) to permanently enable the TCP port.
...
Each Heatmiser device is identified by a unique device name in the hub.
...
| Channel           | Data Type          | Description                                     |
|-------------------|--------------------|-------------------------------------------------|
| sensorTemperature | Number:Temperature | Measured temperature                            |
| batteryLowAlarm   | Switch             | The battery is low (Off=Ok, On=Alarm)           |

## Channel Configuration (Optional)

The Heatmiser devices are connected to the NeoHub by means of an RF (radio frequency) mesh network.
Occasionally, a device might drop out of the mesh.
This is usually a temporary issue (e.g. interference), and the device usually reconnects itself automatically.
If a device drops out of the mesh, you can choose whether openHAB should either a) change the channel value to `UNDEF`, or b) hold the value it had prior to the drop-out.
The choice of a) or b) is determined by an optional Channel configuration parameter `holdOnlineState` as follows.

| Parameter       | Type   | Description                                                                                         |
|-----------------|--------|-----------------------------------------------------------------------------------------------------|
| holdOnlineState | Switch | If the device drops out of the RF mesh, the behavior is:<br> - Off: openHAB changes the channel’s state to `UNDEF`<br> - On: openHAB holds the channel’s state unchanged<br>Default: Off |
...
Note: If a drop-out occurs, the Thing will always change its status to `OFFLINE` (regardless of the `holdOnlineState` setting).
