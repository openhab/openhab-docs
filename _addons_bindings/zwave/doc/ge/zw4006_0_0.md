...
| Motion Alarm  [Deprecated]| alarm_motion | Door | Switch | 
| Switch 1 | switch_binary1 | Switch | Switch | 
| Motion Alarm | alarm_motion2 | Door | Switch | 
...
The ```alarm_motion2``` channel supports the ```Switch``` item and is in the ```Door``` category. This is a read only channel so will only be updated following state changes from the device.
