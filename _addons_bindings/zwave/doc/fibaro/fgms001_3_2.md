...
| Sensor (seismic intensity) | sensor_seismicintensity |  | Number | 
| Sensor (luminance) | sensor_luminance |  | Number | 
| Motion Alarm | alarm_motion | Motion | Switch | 
| Tamper Alarm | alarm_tamper |  | Switch | 
| Battery Level | battery-level | Battery | Number |
| Alarm (general) | alarm_general | Alarm | Switch | 
...
The ```sensor_seismicintensity``` channel supports the ```Number``` item. This is a read only channel so will only be updated following state changes from the device.
...
The ```alarm_motion``` channel supports the ```Switch``` item and is in the ```Motion``` category. This is a read only channel so will only be updated following state changes from the device.
...
| 2 | Flashlight mode - LED glows white for 10 sec |
| 3 | White. |
| 4 | Red. |
| 5 | Green. |
| 6 | Blue. |
