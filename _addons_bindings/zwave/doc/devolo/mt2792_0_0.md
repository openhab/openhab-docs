...
The ```meter_pulse``` channel and is of type ```meter_pulse``` and supports the ```Number``` item and is in the ```Energy``` category. This is a read only channel so will only be updated following state changes from the device.
...
The MT02792 supports 5 association groups.
...
Binary Sensor Command Class: Compatible with 300 Series
  ...
  * Sensor reports the status of the open or closed door via Lifeline.
  * If the sensor detects a status change between closed and open door, the device is triggered.
