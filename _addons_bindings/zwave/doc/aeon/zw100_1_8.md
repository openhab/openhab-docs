...
This version of the device is limited to firmware versions 1.8 to 1.9
...
1. Send **non-security** Node Info frame   Add MultiSensor into Z-Wave network(non-security inclusion):
  ...
  5. press the Action Button. If the LED is still in slow blink, in which you need to repeat the process from
  6. step 2.
...
Remove MultiSensor from Z-wave network:  
  ...
  5. Button. If MultiSensor’s LED still keeps on status, in which you need to repeat the process from step 2.  
...
* [User Manual](https://opensmarthouse.org/zwavedatabase/387/reference/multisensor6.pdf)
